...
public:
  MainGameScreen() {
    ...
    setCameraPos(Utils::vec2{0, 0});

    if (!amongusShader.loadFromFile("shaders/amongus.frag",
                                    sf::Shader::Fragment)) {
    ...
    renderedPlayer =
        std::make_unique<RenderObjects::RenderedPlayer>(&amongusShader);

    coords = std::make_unique<RenderObjects::Text>("XY:", Utils::Transform{},
                                                   Fonts::s_NormalFont, 10);
  }
  ...
  void render(float deltaTime) override {
    ...
    coords->setPos(
        Utils::vec2(s_Renderer->getWindow()->getCamera()->getCenter()) -
        (Utils::vec2(s_Renderer->getWindow()->getCamera()->getSize()) / 2.f) +
        Utils::vec2{10, 10});
    coords->setText("XY: " +
                    std::to_string(renderedPlayer->getPlayer()->pos.x /
                                   Settings::BLOCK_SIZE) +
                    " " +
                    std::to_string(renderedPlayer->getPlayer()->pos.y /
                                   Settings::BLOCK_SIZE));
  ...
  void addOtherPlayer(uint32_t id, GameObjects::PlayerInfo *otherPlayerInfo) {
    if (otherPlayers.find(id) == otherPlayers.end()) {
      otherPlayers[id] = std::make_unique<RenderObjects::RenderedPlayer>(
          &amongusShader, otherPlayerInfo);
    }
  }

  void removeOtherPlayer(uint32_t id) { otherPlayers.erase(id); }

  std::unique_ptr<GameObjects::MultiplayerChunkManager> chunkManager;
  std::unordered_map<uint32_t, std::unique_ptr<RenderObjects::RenderedPlayer>>
      otherPlayers;
...
public:
  IntroScreen() {
    ...
    title = std::make_unique<RenderObjects::Text>(
        "Luntik",
        Utils::Transform(
            Utils::vec2{0, -30}, Utils::vec2{0, 0},
            Utils::Alignment::Alignment2D{Utils::Alignment::MIDDLE,
                                          Utils::Alignment::FRONT}),
        Fonts::s_NormalFont, 32);

    joinButton = std::make_unique<RenderObjects::Button>(
        "JOIN", Fonts::s_NormalFont,
        Utils::Transform(
            Utils::vec2(0, 17), Utils::vec2(130, 25),
            Utils::Alignment::Alignment2D{Utils::Alignment::MIDDLE,
                                          Utils::Alignment::BACK}));

    hostButton = std::make_unique<RenderObjects::Button>(
        "HOST", Fonts::s_NormalFont,
        Utils::Transform(
            Utils::vec2(0, 47), Utils::vec2(130, 25),
            Utils::Alignment::Alignment2D{Utils::Alignment::MIDDLE,
                                          Utils::Alignment::BACK}));
    ...
    textBox = std::make_unique<RenderObjects::TextBox>(
        Fonts::s_NormalFont,
        Utils::Transform(
            Utils::vec2(0, 12), Utils::vec2(130, 25),
            Utils::Alignment::Alignment2D{Utils::Alignment::MIDDLE,
                                          Utils::Alignment::FRONT}),
        10);
  }
...
public:
  DisconnectedScreen() {
    ...
    disconnectedText = std::make_unique<RenderObjects::Text>(
        ...
        Utils::Transform(
            Utils::vec2{0, -10}, Utils::vec2{0, 0},
            Utils::Alignment::Alignment2D{Utils::Alignment::MIDDLE,
                                          Utils::Alignment::FRONT}),
        Fonts::s_NormalFont, 20);

    backButton = std::make_unique<RenderObjects::Button>(
        "BACK", Fonts::s_NormalFont,
        Utils::Transform(
            Utils::vec2(0, 10), Utils::vec2(120, 20),
            Utils::Alignment::Alignment2D{Utils::Alignment::MIDDLE,
                                          Utils::Alignment::BACK}));
  }
...
public:
  PauseScreen() {
    ...
    pauseText = std::make_unique<RenderObjects::Text>(
        "Paused",
        Utils::Transform(
            Utils::vec2{0, -40}, Utils::vec2{0, 0},
            Utils::Alignment::Alignment2D{Utils::Alignment::MIDDLE,
    ...
    backToGameButton = std::make_unique<RenderObjects::Button>(
        "BACK", Fonts::s_NormalFont,
        Utils::Transform(
            Utils::vec2(0, 15), Utils::vec2(130, 25),
            Utils::Alignment::Alignment2D{Utils::Alignment::MIDDLE,
                                          Utils::Alignment::FRONT}));

    mainMenuButton = std::make_unique<RenderObjects::Button>(
        "TO MAIN MENU", Fonts::s_NormalFont,
        Utils::Transform(
            Utils::vec2(0, 25), Utils::vec2(130, 25),
            Utils::Alignment::Alignment2D{Utils::Alignment::MIDDLE,
                                          Utils::Alignment::BACK}));
  }
