...
public:
  ...
  using CallbackOnPacketReceived =
      std::function<void(ID, sf::TcpSocket *, sf::Packet)>;
  ...
  void onClientConnected(CallbackOnClientConnected callback) {
    m_CallbackOnClientConnected = callback;
  }
  void onClientDisconnected(CallbackOnClientDisconnected callback) {
    m_CallbackOnClientDisconnected = callback;
  }

  void setPacketReceiver(Packets::PacketType packetType,
                         CallbackOnPacketReceived callback) {
    m_CallbacksOnPacketReceived[packetType] = callback;
  }
  ...
  void broadcastPacketWithout(const sf::Packet &packet, ID withoutId = -1) {
    for (auto &[id, clientInfo] : m_Clients) {
      ...
      if (clientInfo.socket->send(p) != sf::Socket::Done) {
        LOGGER.log("Broadcasting packet to client: " + std::to_string(id) +
                   " failed!");
      }
    }
  }
...
private:
  ...
  void handleDisconnections() {
    ...
    while (!m_DisconnectedClientsQueue.empty()) {
      ...
      if (m_Clients.find(id) != m_Clients.end()) {
        if (m_Clients.at(id).clientThread.joinable())
    ...
    while (!m_ReceivedPacketsQueue.empty()) {
      ...
      if (packetInfo.packet >> packetType) {
        if (m_CallbacksOnPacketReceived.count(packetType) > 0) {
          m_CallbacksOnPacketReceived.at(packetType)(
              packetInfo.senderID, m_Clients.at(packetInfo.senderID).socket,
              packetInfo.packet);
        }
      } else {
  ...
  void clientThread(ClientInfo *client) {
    while (true) {
      ...
      try {
        ...
        if (receiveStatus != sf::Socket::Done) {
          LOGGER.log("Receive status is not sf::Socket::Done. (Status: " +
                     std::to_string(receiveStatus) + ")");
          break;
        }
    ...
    std::lock_guard<std::mutex> lock(
        this->m_CallbackDisconnectedClientsQueueMutex);
    m_DisconnectedClientsQueue.push(client->id);
    LOGGER.log("Added client " + std::to_string(client->id) +
               " to disconnect queue");
  ...
  void startListen() {
    ...
    LOGGER.log("Listening on port " + std::to_string(m_Port) + ", address " +
               m_Address.toString());
    ...
    m_ListenerThread = std::thread([this]() {
      ...
      while (true) {
        ...
        if (status == sf::Socket::Done) {
          {
            std::lock_guard<std::mutex> lock(
                m_CallbackConnectedClientsQueueMutex);
  ...
  std::unordered_map<Packets::PacketType, CallbackOnPacketReceived>
      m_CallbacksOnPacketReceived;
